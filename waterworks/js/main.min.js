const config = {
  ...
  pixelArt: !0,
  parent: "waterworks",
  pageAlignHorizontally: !0,
  pageAlignVertically: !0,
  scene: {
    preload: preload,
    create: create,
    update: update
...
_defineProperty(this, "unpaused", () => {
  this.gc && (this.gc.paused = !1)
}), _defineProperty(this, "paused", () => {
  this.gc && (this.gc.paused = !0)
}), _defineProperty(this, "runGame", () => {
  (game = new Phaser.Game(config)).onBlur = this.paused, game.onFocus = this.unpaused, game.ontouchend = e => {
    e.preventDefault()
  }
});
var GAME_VERSION = "1.1.6",
  ng = !1,
  ngio; - 1 == window.location.hostname.indexOf("newgrounds.com") && -1 == window.location.hostname.indexOf("ungrounded.net") || (ng = !0);
try {
  ngio = new Newgrounds.io.core("50594:6NeZHUWS", "CSESZK53IrbZy4bMJKbCwA==")
} catch {}
var medals = [],
  CARD_TARGET_BLOCK = "block",
  CARD_TARGET_STREET = "street",
  CARD_TARGET_NODE = "node",
  CARD_TARGET_CONSTRUCTION = "construction",
  CARD_TARGET_GLOBAL = "global",
  CARD_TYPE_WORKER = "worker",
  CARD_TYPE_COMPONENT = "component",
  CARD_TYPE_DOCUMENT = "document",
  CARD_DRAG_DIST_BEFORE_COLLAPSE = 25,
  CARD_SMALL_HEIGHT = 65,
  CARD_BIG_HEIGHT = 140,
  CARD_WIDTH = 100,
  CARD_HAND_ENABLED_Y = 360,
  CARD_HAND_DISABLED_Y = 400,
  STACK_HEIGHT = 138,
  STACK_WIDTH = 98,
  DECK_Y_SHIFT = 105,
  CONSTRUCTION_CARDS_MARGIN_LEFT = 268,
  CONSTRUCTION_CARDS_MARGIN_TOP = 12,
  CONSTRUCTION_CARDS_MARGIN = 6,
  CONSTRUCTION_CARDS_SPACE = 320,
  CONSTRUCTION_CARDS_PER_ROW = 3,
  DRAWER_CARDS_MARGIN_LEFT = 16,
  DRAWER_CARDS_PER_ROW = 4,
  DRAWER_CARDS_PER_LEVEL = 4,
  DRAWER_CARDS_MARGIN = 2,
  DRAWER_CARDS_MARGIN_TOP = 20,
  ...
  POPUP_Y2 = 170,
  LAYER_FIRE = 800,
  LAYER_SUPPLY_DEMAND = 900,
  LAYER_GUI_LOW = 1e3,
  LAYER_CONSTRUCTION = 1200,
  LAYER_CURTAIN = 1250,
  LAYER_CONSTRUCTION_ACTIVE = 1300,
  LAYER_MONEY_INF_COUNTER = 1400,
  LAYER_DRAWER_CLOSED = 1500,
  LAYER_CARD = 1550,
  LAYER_DRAWER_OPEN = 1600,
  LAYER_GUI = 2e3,
  LAYER_GUI_ACTIVE = 2100,
  LAYER_PARTICLES = 2300,
  LAYER_CARD_DRAG = 2400,
  LAYER_TUTORIAL = 2500,
  LAYER_GUI_MAIN = 2600,
  LAYER_GUI_OPTIONS = 2700,
  LAYER_POPUP = 3e3,
  LAYER_TOOLTIP = 4e3,
  WATER_SOURCE = 16,
  PP_LEV_AGE_1 = 1,
  ...
  PP_INC_AGE_2 = 2,
  PP_INC_AGE_3 = 3,
  ACH_COINS_TO_COLLECT = 5e3,
  ACH_TIME_LIMIT = 130,
  ACH_REQUESTS_TO_MAKE = 20,
  ACH_PERFECT_TURNS = 3,
  privacy_popup, privacy_text, privacy_text2, privacy_link, privacy_link2, privacy_accept, privacy_pl_flag, privacy_en_flag, self;

function preload() {
  var progressBar = this.add.graphics(),
    progressBox = this.add.graphics();
  progressBox.fillStyle(2236962, .8), progressBox.fillRect(GAME_WIDTH / 2 - 160, GAME_HEIGHT / 2 + 50, 320, 50);
...
function create() {
  ...
  self = this, this.anims.create({
    ...
    key: "boat1",
    frames: this.anims.generateFrameNames("boats", {
      prefix: "boats",
      start: 0,
      end: 15
    }),
    ...
    key: "mill1",
    frames: this.anims.generateFrameNames("mill_atlas", {
      prefix: "mill",
      start: 0,
      end: 15
    }),
    ...
    key: "water_source",
    frames: this.anims.generateFrameNames("water_source", {
      prefix: "water_source",
      start: 0,
      end: 39
    }),
    ...
    key: "water_source2",
    frames: this.anims.generateFrameNames("water_source2", {
      prefix: "water_source2",
      start: 0,
      end: 39
    }),
    repeat: -1,
    ...
    key: "flow1",
    frames: this.anims.generateFrameNames("river1_atlas", {
      prefix: "river",
      start: 0,
      end: 31
    }),
    ...
    key: "flow2",
    frames: this.anims.generateFrameNames("river2_atlas", {
      prefix: "river",
      start: 0,
      end: 31
    }),
    ...
    key: "horse_walk_right",
    frames: this.anims.generateFrameNames("horse_atlas", {
      prefix: "horse",
      start: 0,
      end: 23
    }),
    ...
    key: "horse_hanging",
    frames: this.anims.generateFrameNames("hanginghorse_atlas", {
      prefix: "hanginghorse",
      start: 0,
      end: 15
    }),
    ...
    key: "wagon_walk_right",
    frames: this.anims.generateFrameNames("wagon_atlas", {
      prefix: "wagon",
      start: 0,
      end: 23
    }),
    ...
    key: "carrier_walk_right",
    frames: this.anims.generateFrameNames("carrier_atlas", {
      prefix: "carrier",
      start: 0,
      end: 23
    }),
    ...
    key: "hourglass_anim",
    frames: this.anims.generateFrameNames("gui", {
      prefix: "hourglass_anim",
      start: 1,
      end: 10
    }),
    repeat: 0,
    ...
    key: "waterworks1",
    frames: this.anims.generateFrameNames("waterworks", {
      prefix: "waterworks",
      start: 0,
      end: 15
    }),
...
function onMedalsLoaded(result) {
  result.success && (medals = result.medals)
}

function startGame() {
  this.add.image(320, 180, "background"), this.scale.pageAlignHorizontally = !0, this.scale.pageAlignVertically = !0, this.gc = new GameController(this)
}

function getFirstElementByAttributeValue(xml, attribute, value) {
  for (var i = 0; i < xml.length; i++)
    if (xml[i].getAttribute(attribute) == value) return xml[i];
  return null
}

function update(time, delta) {
  this.gc && this.gc.update(time, delta)
}

function enableChildren(child) {
  child.input && (null != child.previouslyEnabled && (child.input.enabled = child.previouslyEnabled), child.input.enabled = !0, child.previouslyEnabled = child.input.enabled)
}

function disableChildren(child) {
  child.input && (child.previouslyEnabled = child.input.enabled, child.input.enabled = !1)
}

function getString(language, tag, id, key) {
  var xmlData = game.cache.xml.get("strings_" + language.toLowerCase()).getElementsByTagName(tag);
  return (xmlData = getFirstElementByAttributeValue(xmlData, "id", id)).getElementsByTagName(key)[0].childNodes[0].nodeValue
}
...
function openExternalLink(externalLink) {
  var s = window.open(externalLink, "_blank");
  s && s.focus ? s.focus() : s || (window.location.href = externalLink)
}
